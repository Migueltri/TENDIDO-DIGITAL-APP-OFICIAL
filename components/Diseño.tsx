...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navItems: { to: string; icon: React.ReactNode; label: string; hidden?: boolean }[] = [
    ...
    { to: '/instalar', icon: <Smartphone size={20} />, label: 'Descargar App', hidden: false },
  ];
      ...
      `}>
        ...
        <div className="p-4 border-t border-gray-700 space-y-3">
          <a 
            ...
            className="flex items-center justify-center gap-2 px-4 py-3 bg-[#1DB954] hover:bg-[#1ed760] text-white rounded-lg transition-colors font-bold shadow-lg"
          >
            <Globe size={18} />
            IR A WEB OFICIAL
          </a>
          <NavLink 
            to="/ver-web" 
            onClick={() => setIsMobileMenuOpen(false)}
            className="flex items-center justify-center gap-2 px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors text-sm"
          >
            <Eye size={16} />
            Ver Simulación
          </NavLink>
